<template>
    <div id="courseAllList">

        <div style="background-color: #f3f4f6;" :style="contontHeight" class="list">
            <el-row>

        <!-- <div style="background-color: #f3f4f6;min-height: 807px;" class="list"> -->
            <!-- <el-row> -->
                <!-- 顶部条件筛选 后期需要-->
                ...
                <el-col :span="24" style="display:flex;justify-content:center;">
                    <el-col style="width:1248px;margin-left:48px">
                        <div style="float: left;margin-bottom:77px;margin-right:48px;width: 264px;height: 156px;display:inline-block" v-for="(item,index) in courseList" :key="index">
                            <div style="float:left;width: 264px;height: 156px;border-radius: 5px;display:block;">
                                <img v-if="item.fileId" :src="previewCourse+item.fileId" class="image_bg" @click="toJumpCourse(item, index)" title="点击查看章节">
                                <img v-else src="@/assets/images/coverPic.jpg" class="image_bg" @click="toJumpCourse(item, index)" title="点击查看章节">
        ...
        methods:{
            getCourseAllData(msg){
                this.$store.dispatch('courseAllList/courseListAct',msg).then(() => {
                    this.courseList = this.$store.state.courseAllList.courseList;
                    this.allInfo = this.$store.state.courseAllList.allInfo;
                   // console.log(this.courseList, this.allInfo,'--课程列表--');
                }).catch(() => {
                })
            },
...
<style >
   ...
   .coursePagination .el-pagination.is-background .el-pager li:not(.disabled).active{
        border-radius: 50%;
        background-color: #f49217 !important;
    }
    .coursePagination .el-pagination.is-background .btn-next, .coursePagination .el-pagination.is-background .btn-prev, .coursePagination  .el-pagination.is-background .el-pager li{
        background-color: #F3F4F6 !important;
    }
    .coursePagination .el-pagination.is-background .el-pager li:not(.active):hover{
