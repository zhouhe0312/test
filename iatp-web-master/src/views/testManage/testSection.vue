            ...
            <el-row>
                <el-col :span="24" style="display:flex;justify-content:center;background:#fff">
                    <el-col style="width:1200px">
                        ...
                        <span> > </span>
                        <router-link to="/testAllList" class="topJump">全部实验</router-link>
                        <span> > </span>
                ...
                <el-col :span="24" style="background-color: #f0f2f5" >
                    <div style="display:flex;flex-direction:row;flex-wrap:wrap;justify-content:center;">
                        <div style="display:flex;flex-direction:row;flex-wrap:wrap;justify-content:center;width:100%;background:#fff">
                            <div style="width:1200px;margin-bottom:30px;margin-top:30px">
                                ...
                                <div style="text-align: left;height: 240px;width: 69%;margin-left: 442px;margin-top: 5px;">
                                    <div style="overflow:hidden;text-overflow: ellipsis;white-space:nowrap;width:93%">
                        ...
                        <div style="width:1200px;background-color: #f0f2f5;min-height:400px">
                            <p style="float:left"><i class="el-icon-s-unfold" style="margin:18px 0px 5px 5px"></i>实验章节</p>
                            <div class="chapter" v-for="(item,index) in testList" :key="index">
                                ...
                                <div class="_chapter" v-for="(data,_index) in item.child" :key="_index" @click="jumpCourseStudy(data)">
    ...
    export default {
        ...
        methods:{
            ...
            jumpCourseStudy(data){
                setValue('TESTSECTIONRESOURCE',data)
                 this.$router.push({
            ...
            getCourseListAct(id){
                ...
                this.$store.dispatch('personal/detailsAct').then(() => {
                    ...
                    this.$store.dispatch('courseSection/getCourseListAct',val).then(() => {
                        this.testList = this.$store.state.courseSection.courseList;
                        console.log(this.sectionList,'--实验章节列表--');
                    }).catch(() => {
                    })
                }).catch(() => {
                    this.$store.dispatch('courseSection/getCourseListAct',val).then(() => {
                ...
                // this.$store.dispatch('courseSection/getCourseListAct',id).then(() => {
                //     this.testList = this.$store.state.courseSection.courseList;
                //     console.log(this.testList,'--实验章节列表--');
                // }).catch(() => {
            ...
            getSectionDetail(id){
                this.$store.dispatch('courseSection/getCourseDetailAct',id).then(() => {
                   ...
                   // console.log(this.sectionDetails,'--实验详情--');
                }).catch(() => {
...
<style>
    ...
    ._chapter:hover, ._chapter:hover._chapter>p{
